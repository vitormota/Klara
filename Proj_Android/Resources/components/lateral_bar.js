...
function LateralBarWithoutSearch()
{
	...
	this.constructorLateralBar = function()
	{
		...
		if(fb.loggedIn)
		{
			...
			/*** Botão Logout***/
			this.lateral_logout_view = Titanium.UI.createView({
				top: '74.32%',
				left: '17.07%',
				backgroundColor: '#f1e9cE',
				width: '71.68%',
			...
			var lateral_logout_text = Titanium.UI.createLabel({
				text: "LOGOUT",
				verticalAlign: Titanium.UI.TEXT_VERTICAL_ALIGNMENT_CENTER,
				width: '53.21%',
				left: '32.44%',
			...
			this.lateral_logout_view.add(lateral_logout_image);
			this.lateral_logout_view.add(lateral_logout_text);
			this.lateral_bar.add(this.lateral_logout_view);
		}
		else
		{
			/*** Botão Home***/
